...
void ClientListModel::setNetworkClient(NetworkClient* networkClient)
{
	...
	connect(d->networkClient, &NetworkClient::clientEnabledVideo, d.data(), &ClientListModelPrivate::onClientEnabledVideo);
	connect(d->networkClient, &NetworkClient::clientDisabledVideo, d.data(), &ClientListModelPrivate::onClientDisabledVideo);
	d->clients.clear();
	endResetModel();
}
...
void ClientListModel::updateClient(const ClientEntity& client)
{
	for (auto i = 0; i < d->clients.size(); ++i)
	{
		if (d->clients[i].id == client.id)
		{
			d->clients[i] = client;
...
void ClientListModelPrivate::onClientEnabledVideo(const ClientEntity& client)
{
	auto d = this;
	d->owner->updateClient(client);
}

void ClientListModelPrivate::onClientDisabledVideo(const ClientEntity& client)
{
	auto d = this;
	d->owner->updateClient(client);
}
