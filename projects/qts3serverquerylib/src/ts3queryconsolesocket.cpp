...
bool TS3QueryConsoleSocketSync::updateNickname(const QString& nickname)
{
	TS3ServerQuery sq;
	TS3QueryConsoleCommandData c;
	c.params << qMakePair(QString("client_nickname"), QStringList() << nickname);
	c.cmd = sq.createCommand("clientupdate", c.params, c.options);
	write(c.cmd.toUtf8());
