...
char* findClientExeFilePathInRegistry()
{
	...
	result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\mfreiholz.de\\Conference Client", 0, KEY_READ, &hkey);
	if (result != ERROR_SUCCESS)
	{
		result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\WOW6432Node\\mfreiholz.de\\Conference Client", 0, KEY_READ, &hkey);
		if (result != ERROR_SUCCESS)
			return 0;
	}
