...
/**
 * Concatenates the channel hierachy into one string.
 */
char* getChannelPath( uint64 serverConnectionHandlerID, uint64 channelID )
{
  ...
  if( ts3Functions.getChannelVariableAsString( serverConnectionHandlerID, channelID, CHANNEL_NAME, &channelname ) != ERROR_ok )
    return NULL;
  strcat( path, channelname );
  ts3Functions.freeMemory( channelname );

  // Get first parent channel ID.
  if( ts3Functions.getParentChannelOfChannel( serverConnectionHandlerID, channelID, &parentid ) != ERROR_ok )
    return NULL;

  while( parentid > 0 ) {
    // Get channel name.
    if( ts3Functions.getChannelVariableAsString( serverConnectionHandlerID, parentid, CHANNEL_NAME, &channelname ) != ERROR_ok )
      break;

    _strcpy( buffer, 1024, path );
    path[0] = '\0';
    ...
    // Get next parent channel ID.
    if( ts3Functions.getParentChannelOfChannel( serverConnectionHandlerID, parentid, &parentid ) != ERROR_ok )
      break;
  }
 ...
 *  0 = success
 *  1 = failure
 * -2 = failure but client will not show a "failed to load" warning.
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) {
  return 0;  // Plugin didn't handle command.
}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
  ...
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_CHANNEL_1, "Join channel wall", "camera.png");
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL,  MENU_ID_GLOBAL_1, "About StreamWall Plugin", "camera.png");
  ...
  *menuIcon = (char*)malloc(PLUGIN_MENU_BUFSZ * sizeof(char));
  _strcpy(*menuIcon, PLUGIN_MENU_BUFSZ, "camera.png");
}
 ...
 * IMPORTANT: Don't implement without creating hotkeys. Results in TS crash.
 */
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
  ...
  switch(type) {
    ...
    case PLUGIN_MENU_TYPE_CHANNEL:
      ...
      switch(menuItemID) {
        case MENU_ID_CHANNEL_1: // Hangout item clicked.
          // Get user ID.
          if( ts3Functions.getClientID( serverConnectionHandlerID, &clientid ) != ERROR_ok )
            break;

          // Get user name.
          if( ts3Functions.getClientSelfVariableAsString( serverConnectionHandlerID, CLIENT_NICKNAME, &clientname ) != ERROR_ok )
            break;

          // Get channel name.
          channelname = getChannelPath( serverConnectionHandlerID, selectedItemID );
          if( !channelname )
            break;

          // Get server IP and port. CONNECTION_SERVER_IP CONNECTION_SERVER_PORT
          if( ts3Functions.getConnectionVariableAsString( serverConnectionHandlerID, clientid, CONNECTION_SERVER_IP, &ip ) != ERROR_ok )
            break;

          if( ts3Functions.getConnectionVariableAsUInt64( serverConnectionHandlerID, clientid, CONNECTION_SERVER_PORT, &port ) != ERROR_ok )
            break;

          // Start hangout.
          //startHangout( ip, port, channelname, clientname );
